...
class WideResNet(nn.Module):
    ...
    #     # out = self.pool(out) # expected size batch x 128 x 1 x 1
    ...
    def forward(self, x):
        ...
        # print(f"x shape 0: {x.shape}") # torch.Size([2, 3, 512, 512])
        out = self.conv1(x)
        out = self.block1(out)
        out = self.block2(out)
        out = self.block3(out)
        ...
        # out = F.avg_pool2d(out, 8)
        out = self.pool(out) 
        print(f"x shape after pooling: {out.shape}") # torch.Size([bs, 128, 1, 1])
        # pdb.set_trace()
        out = out.view(-1, self.nChannels) 
    ...
    def feature_list(self, x):
        print(f"feature list is called?!!")
        out_list = [] 
        out = self.conv1(x)
        out = self.block1(out)
        out = self.block2(out)
